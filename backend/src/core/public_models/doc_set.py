...
class DocumentSet(CamelModel):
    ...
    status: DocumentSetStatus = Field(
        description="Status.",
    )
    is_new_doc_default: bool = Field(
        description="True if default document set for new documents"
    )
    is_public_viewable: bool = Field(
        description="True if documents are publicly visible"
    )
...
class DocumentSetUpdateRequest(CamelModel):
    ...
    is_public_viewable: Optional[bool] = Field(
        description="True if documents are publicly visible",
        default = None
    )
