...
def get_s3_bucket() -> S3Path:
    ...
    return S3Path(f's3://{config.s3_bucket_name}/', client=client)
...
async def ping_file_store() -> PingResult:
    ...
    try:
        ...
        logger.error('Error pinging S3', exc_info=e)
        return PingResult(status=PingStatus.BAD, message='S3 connection failed.', error=str(e))
