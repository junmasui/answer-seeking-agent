...
class LLMMetrics:
    ...
    def _init_metrics(self):
        ...
        self.token_counter = self.meter.create_counter(
            ...
            description="Total number of tokens processed by LLMs",
            unit="1"
        )
        ...
        if quality_score is not None:
            ...
            if not hasattr(self, 'quality_histogram'):
                self.quality_histogram = self.meter.create_histogram(
                    ...
                    description="Quality score of LLM responses (0.0-1.0)",
                    unit="1"
                )
        ...
        class OperationTracker:
            ...
            def record_cost(self, cost_usd: float):
                self.metrics.record_cost(
                    cost_usd, self.model, self.provider, operation_name, self.session_id
                )
            
            def record_error(self, error_type: str, error_message: Optional[str] = None):
                self.metrics.record_error(
                    error_type, self.model, self.provider, error_message, self.session_id
                )
