...
async def delete_document_set(document_set_id):
    """Delete document set."""
    # Retrieve tracking record.

    doc_set_records = await get_document_sets(doc_set_uuid_list=[document_set_id])
    ...
    await delete_tracking_record(doc_set_record.id)
