...
sudo curl -fsSL --proto-redir https https://download.docker.com/linux/debian/gpg -o /etc/apt/keyrings/docker.asc
...
This environment uses `dev-tools` as the Mutagen hub. The host bind-mounts the codebase into `dev-tools`, and `dev-tools` runs the Mutagen client to push one-way updates into other services over SSH. The host does not run Mutagen.
...
- Private key for the `dev-tools` Mutagen client: [services/secrets/mutagen.private_key](services/secrets/mutagen.private_key)
...
minio-init-1  | Added user `agent` successfully.
minio-init-1  | Attached Policies: [readwrite]
minio-init-1  | To User: agent
```
...
pgvector-init-1  | You are now connected to database "agent" as user "postgres".
...
```bash
docker compose logs webui-server
```
...
```console
webui-server-1  | 
webui-server-1  | added 252 packages, and audited 253 packages in 7s
webui-server-1  | 

...

webui-server-1  | > vite --host 0.0.0.0 --logLevel info
webui-server-1  | 
webui-server-1  | 
webui-server-1  |   VITE v5.4.11  ready in 885 ms
webui-server-1  | 
webui-server-1  |   ➜  Local:   http://localhost:5173/
webui-server-1  |   ➜  Network: http://172.31.1.7:5173/
webui-server-1  |   ➜  Vue DevTools: Open http://localhost:5173/__devtools__/ as a separate window
webui-server-1  |   ➜  Vue DevTools: Press Alt(⌥)+Shift(⇧)+D in App to toggle the Vue DevTools
...
agent-webui-server-1      webui-server      Up About an hour
```
