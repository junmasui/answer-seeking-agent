...
def register_initial_prompts(sender):
    ...
    Loads predefined prompts from initial_prompts.yml and adds them to the database when the
    database is ready for predefined data. Only runs on non-worker processes.
    """
    if sender.is_worker:
        return
