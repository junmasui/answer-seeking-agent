...
async def execute_nemo_guardrails_check(config_id: str, messages: list) -> dict:
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(guardrails_url, json=payload, headers=headers, timeout=60.0)
